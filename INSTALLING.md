...
|`PROVIDER_ACCOUNT_SEED_PHRASE`| **yes***  | Seed phrase for provider control keypair (required only when `ETHEREUM_PROVIDER_ACCOUNT_PRIVATE_KEY` is not set). This is for backwards compatibility for Sr25519 keys. |_none_|
|`ETHEREUM_PROVIDER_ACCOUNT_PRIVATE_KEY`| **yes***  | Private key for provider control ethereum keypair (if set it will override `PROVIDER_ACCOUNT_SEED_PHRASE`).                       |_none_|
